...
export function CartProvider({ children }) {
    ...
    setItems(prev => {
      ...
      return [
        ...prev,
        {
          ...
          // %40 indirim politikasına göre sepette ödenecek fiyat
          discounted: Math.round((urun.salePrice || urun.price) * 0.6),
          image: urun.images?.[0],
  ...
  // toplamı indirimli fiyat üzerinden hesapla
  const total = useMemo(
    () => items.reduce((acc, it) => acc + it.discounted * it.qty, 0),
    [items],
  );
  const count = useMemo(
